//! The on-disk file layer ported from `packages/coducktor/src/{paths,config,workspace}.ts`.
//!
//! The source of truth for each module's behavior remains the matching TypeScript file —
//! every public item cites it in a doc comment. `packages/coducktor` keeps running the server
