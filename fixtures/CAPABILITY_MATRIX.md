...
| Review / approval | `review-mode` | `failed-and-denied` (same `--permission-mode dontAsk` denial path as Question/permission — Claude has no separate review-mode concept) | unverified — no permission/approval event of any kind is recognized; an unrecognized bus event is silently dropped by the mapper's fallback, a real hang risk if the live server's permission policy is ever not auto-approve | n/a — no review-mode event exists in pi's RPC vocabulary |
| Question / permission | explicit JSON-RPC decline or durable park | `failed-and-denied` (headless answer seam unsupported) | `session-error` | the question half is handled runner-neutrally via the `DUCK:ASK` marker, tested by `coducktor-core`'s own `runs::ask` unit tests rather than a golden fixture here; the permission half is n/a — no interactive tool-approval RPC exists in pi's protocol |
