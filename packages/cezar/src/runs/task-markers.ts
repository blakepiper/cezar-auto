 ...
 * same way it declares completion with `DUCK:DONE`. Parsed from the accumulated
...
// fragment's own `DUCK:PR=<number>` placeholder is non-numeric and inert.
//
// DUAL-READ SHIM (spec §2.2.2): the parser accepts BOTH `DUCK:*` (what cezar
// emits now) and the legacy `CEZ:*` spelling, because in-flight sessions and
// unrewritten skills still emit the old form. The alternation is the whole
// cost; a hard cut would break every live run and skill for no benefit.
const PR_MARKER_RE = /^(?:CEZ|DUCK):PR=(\d+)\s*$/gm;
const ISSUE_MARKER_RE = /^(?:CEZ|DUCK):ISSUE=(\d+)\s*$/gm;
const TITLE_MARKER_RE = /^(?:CEZ|DUCK):TITLE=(.+)$/gm;
...
// DUCK:PR / DUCK:ISSUE in the same turn wins. The skill docs' own placeholders
...
const MARKER_LINE = /^(?:CEZ|DUCK):(?:PR=\d+|ISSUE=\d+|TITLE=.+)\s*$/;

/**
 * Remove complete marker lines from display text — the `stripDoneMarker`
 * precedent. Only control lines (`DUCK:*` and its legacy `CEZ:*` twin) are
 * stripped: the report-tier reference lines (`PR: #12 (link: …)`) are
 * human-readable by design and stay visible. Best-effort by design: a marker
 * split across streamed v1 chunks may transiently render; parsing always runs
 * on the whole turn text, so the record is never affected. Mirrored for v2
 * display in the cockpit's `thread-state.ts`.
 */
export function stripTaskMarkers(text: string): string {
  if (!text.includes('CEZ:') && !text.includes('DUCK:')) return text;
  return text
    .split('\n')
